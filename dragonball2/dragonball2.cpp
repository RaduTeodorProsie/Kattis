...
vector<pair<int, pair<int, bitset<NMAX>>>>::iterator
getcost(const pair<int, bitset<NMAX>> &s, const int &unde) {
  for (auto it = h[unde].begin(); it != h[unde].end(); it++)
    if (it->second.second == s.second)
      return it;
  return h[unde].end();
}
...
class cmp {
public:
  bool operator()(const pair<int, pair<int, bitset<NMAX>>> &a,
                  const pair<int, pair<int, bitset<NMAX>>> &b) {
...
ll dijkstra() {
  int u = has({1, info[1]});
  h[u].push_back({0, {1, info[1]}});
  priority_queue<pair<int, pair<int, bitset<NMAX>>>,
                 vector<pair<int, pair<int, bitset<NMAX>>>>, cmp>
      heap;
  heap.push({0, {1, info[1]}});
  while (!heap.empty()) {
    auto v = heap.top();
    heap.pop();
    if (v.second.second.count() >= 7)
      return v.first;
    for (auto &it : vecini[v.second.first]) {
      int nc = v.first + it.second;
      bitset noua = v.second.second;
      noua |= info[it.first];
      pair<int, bitset<NMAX>> g{it.first, noua};
      int unde = has(g);
      vector<pair<int, pair<int, bitset<NMAX>>>>::iterator caut =
          getcost(g, unde);
      if (caut == h[unde].end()) {
